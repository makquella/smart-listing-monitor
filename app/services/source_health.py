...
class SourceHealthService:
    ...
    def evaluate(self, *, item_count: int, warnings: list[str], recent_healthy_counts: list[int]) -> HealthEvaluation:
        ...
        if warnings:
            return HealthEvaluation(status="degraded", parse_completeness_ratio=1.0 if baseline is None else item_count / baseline)
